...
export default function AdminRequestDetailPage() {
  ...
  const [paymentApprovalSuccess, setPaymentApprovalSuccess] =
    useState("");

  const [sendingFinalDocument, setSendingFinalDocument] =
    useState(false);
  const [sendFinalDocumentError, setSendFinalDocumentError] =
    useState("");
  const [sendFinalDocumentSuccess, setSendFinalDocumentSuccess] =
    useState("");

  const [finalizingRequest, setFinalizingRequest] =
    useState(false);
  const [finalizeRequestError, setFinalizeRequestError] =
    useState("");
  const [finalizeRequestSuccess, setFinalizeRequestSuccess] =
    useState("");
  ...
  async function approvePayment() {
    try {
      ...
      const response = await fetch(
        ...
        {
          method: "POST",
          headers: { "x-saudeclick-context": "admin" },
        },
      );
  ...
  async function sendFinalDocumentToPatient() {
    if (!data || !latestFinalDocument) {
      setSendFinalDocumentError(
        "Anexe o documento final antes de enviá-lo.",
      );
      return;
    }

    try {
      setSendingFinalDocument(true);
      setSendFinalDocumentError("");
      setSendFinalDocumentSuccess("");
      ...
      if (!contentType.includes("application/json")) {
        throw new Error(
          response.status === 404
            ? "A API de envio do documento final não foi encontrada."
      ...
      setSendFinalDocumentError(
        requestError instanceof Error
          ? requestError.message
          : "Erro ao enviar o documento final.",
      );
    } finally {
      setSendingFinalDocument(false);
    }
  }
  ...
  async function finalizeRequest() {
    ...
    try {
      ...
      setFinalizeRequestError("");
      setFinalizeRequestSuccess("");
      ...
      if (
        ...
        throw new Error(
          response.status === 404
            ? "A API de finalização não foi encontrada."
            : `A API respondeu em formato inesperado (${response.status}).`,
        );
      }
  ...
  useEffect(() => {
    ...
    const events =
      new EventSource(
        `/api/realtime/solicitacoes/${encodeURIComponent(
          requestId,
        )}`,
      );
  ...
  if (loading) {
    return (
      <main className="saudeclick-shell flex min-h-screen items-center justify-center">
  ...
  if (error || !data) {
    return (
      <main className="saudeclick-shell flex min-h-screen items-center justify-center px-6">
  ...
  return (
    <main className="saudeclick-shell relative min-h-screen overflow-x-hidden text-slate-100">
              ...
              <section className="relative overflow-hidden rounded-[22px] border border-emerald-200/80 bg-gradient-to-b from-emerald-50/90 via-white to-white shadow-[0_12px_35px_rgba(15,118,110,0.08)]">
                ...
                <div className="relative space-y-2 p-3">
                  ...
                  <FlowButtonStep
                    ...
                    description="Disponibilize o documento final ao paciente no portal."
                    icon={Send}
                    done={finalSent}
                    disabled={
                      !latestFinalDocument ||
                    ...
                    onClick={() =>
                      void sendFinalDocumentToPatient()
                    }
                    tone="blue"
                    feedback={
                      sendFinalDocumentError ||
                      sendFinalDocumentSuccess
                    }
